...
class SendStgCmtResult extends TimerTask {
    ...
    public SendStgCmtResult(DcmRcv dcmrcv, NetworkApplicationEntity stgcmtAE, DicomObject result) {
        this.dcmrcv = dcmrcv;
    ...
    public void run() {
        try {
            ...
            DcmRcv.LOG.warn("Send Storage Commitment Result to " + stgcmtAE.getAETitle() + " failed:", e);
            if (failureCount++ < dcmrcv.getStgCmtRetry()) {
                DcmRcv.LOG.info("Schedule retry in " + (dcmrcv.getStgCmtRetryPeriod() / 1000) + "s.");
                return;
            }
        }
        this.cancel();
