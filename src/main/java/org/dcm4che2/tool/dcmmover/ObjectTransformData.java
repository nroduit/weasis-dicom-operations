...
/**
 * Container for the data used to anonymize the DICOM objects being moved. Restrictive about what attributes can be
 * modified but wide open about attributes that can be removed.
 * 
...
public class ObjectTransformData {
    ...
    public void setPatientBirthDate(String d_str) {
        Date date = DateUtil.getDate(d_str);
        if (date != null) {
    ...
    public void setPatientBirthTime(String d_str) {
        Date time = DateUtil.getTime(d_str);
        if (time != null) {
            setPatientBirthTime(time);
        }
    }
